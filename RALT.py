...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/raimundbuehler/Desktop/RALT_english/RALT.py',
...
CycleText = ''
CycleInfo = visual.TextStim(win=win, name='CycleInfo',
    text='default text',
    font=font_choice,
...
for thisTraining_social in training_social:
    ...
    # keep track of which components have finished
    trialComponents = [fix_cross, new_neutral, response_training, Stimulus, left_disp, right_disp]
    ...
    while continueRoutine:
        ...
        if new_neutral.status == STARTED:
            ...
            if tThisFlipGlobal > new_neutral.tStartRefresh + 6-frameTolerance:
                ...
                win.timeOnFlip(new_neutral, 'tStopRefresh')  # time at next scr refresh
...
for thisBlock in blocks:
    ...
    for thisCycle in cycles:
        ...
        _key_resp_6_allKeys = []
        # keep track of which components have finished
        LateralizationByCycleComponents = [CycleInfo, key_resp_6]
        ...
        while continueRoutine:
            ...
            # *CycleInfo* updates
            if CycleInfo.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                CycleInfo.frameNStart = frameN  # exact frame index
                CycleInfo.tStart = t  # local t and not account for scr refresh
                CycleInfo.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(CycleInfo, 'tStartRefresh')  # time at next scr refresh
        ...
        for thisComponent in LateralizationByCycleComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        cycles.addData('CycleInfo.started', CycleInfo.tStartRefresh)
        cycles.addData('CycleInfo.stopped', CycleInfo.tStopRefresh)
        ...
        for thisTrial in trials:
            ...
            new_neutral_2 = visual.MovieStim3(
                win=win, name='new_neutral_2',units='height', 
                noAudio = True,
                filename=filename_thisN_neutral,
                ori=0, pos=[0, 0.225], opacity=1,
                loop=False,
                size=[0.625, 0.5],
                depth=-2.0,
            ...
            # keep track of which components have finished
            trial_2Components = [fix_cross_2, new_neutral_2, response_training_2, Stimulus_2, left_disp_2, right_disp_2]
            ...
            while continueRoutine:
                ...
                if new_neutral_2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                    # keep track of start time/frame for later
                    new_neutral_2.frameNStart = frameN  # exact frame index
                    new_neutral_2.tStart = t  # local t and not account for scr refresh
                    new_neutral_2.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(new_neutral_2, 'tStartRefresh')  # time at next scr refresh
                    new_neutral_2.setAutoDraw(True)
                if new_neutral_2.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > new_neutral_2.tStartRefresh + 6-frameTolerance:
                        # keep track of stop time/frame for later
                        new_neutral_2.tStop = t  # not accounting for scr refresh
                        new_neutral_2.frameNStop = frameN  # exact frame index
                        win.timeOnFlip(new_neutral_2, 'tStopRefresh')  # time at next scr refresh
                        new_neutral_2.setAutoDraw(False)
                if new_neutral_2.status == FINISHED:  # force-end the routine
                    continueRoutine = False
